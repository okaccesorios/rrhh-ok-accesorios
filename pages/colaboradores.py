import streamlit as st
from utils.database import get_conn, dict_cursor, log_auditoria
from utils.auth import usuario_actual, puede
from datetime import date

SECTORES = ["Administración","Compras","Montecaseros","Local calle San Juan","Logistica"]
TIPOS = ["efectivo","externo","eventual"]
TIPO_LABEL = {"efectivo":"👔 Efectivo","externo":"🔗 Externo","eventual":"📋 Eventual"}

def show():
    st.markdown('<div class="main-header"><div><h1>👥 Colaboradores</h1><span>Gestión de la plantilla</span></div></div>', unsafe_allow_html=True)
    u = usuario_actual()
    tab1, tab2 = st.tabs(["👥 Listado", "➕ Nuevo / Editar"])

    with tab1:
        col1, col2 = st.columns(2)
        with col1: filtro_sector = st.selectbox("Sector", ["Todos"]+SECTORES)
        with col2: filtro_activo = st.selectbox("Estado", ["Activos","Inactivos","Todos"])

        conn = get_conn()
        c = dict_cursor(conn)
        sql = "SELECT * FROM colaboradores WHERE 1=1"
        params = []
        if filtro_sector != "Todos": sql += " AND sector=%s"; params.append(filtro_sector)
        if filtro_activo == "Activos": sql += " AND activo=1"
        elif filtro_activo == "Inactivos": sql += " AND activo=0"
        sql += " ORDER BY sector, apellido"
        c.execute(sql, params)
        rows = c.fetchall()
        conn.close()

        for r in rows:
            activo_badge = "🟢" if r["activo"] else "🔴"
            tipo_badge = {"externo":"🔗","eventual":"📋"}.get(r.get("tipo","efectivo"),"👔")
            rot_txt = " · rotativo" if r["rotativo"] else ""
            with st.expander(f"{activo_badge} {tipo_badge} **{r['legajo']}** — {r['apellido']} {r['nombre']} | {r['sector']}"):
                c1, c2, c3 = st.columns(3)
                with c1:
                    st.markdown(f"**Legajo:** {r['legajo']}")
                    st.markdown(f"**Sector:** {r['sector']}")
                    st.markdown(f"**Turno:** {r.get('turno') or 'Fijo'}{rot_txt}")
                    st.markdown(f"**Tipo:** {TIPO_LABEL.get(r.get('tipo','efectivo'),'efectivo')}")
                with c2:
                    st.markdown(f"**Entrada:** {r['entrada']}  |  **Salida:** {r['salida']}")
                    st.markdown(f"**Sábado:** {r['entrada_sab']} — {r['salida_sab']}")
                    st.markdown(f"**Break:** {r['break_min']}min  |  **Almuerzo:** {r['almuerzo_min']}min")
                with c3:
                    if puede("colaboradores") and u["rol"] in ("admin","rrhh"):
                        nuevo_estado = 0 if r["activo"] else 1
                        label = "🔴 Dar de baja" if r["activo"] else "🟢 Reactivar"
                        if st.button(label, key=f"baja_{r['id']}"):
                            conn2 = get_conn(); c2b = dict_cursor(conn2)
                            c2b.execute("UPDATE colaboradores SET activo=%s WHERE id=%s",
                                        (nuevo_estado, r["id"]))
                            conn2.commit(); conn2.close()
                            log_auditoria(u["username"],"TOGGLE_ACTIVO","colaboradores",r["id"])
                            st.rerun()

    with tab2:
        st.markdown("#### Nuevo colaborador o modificar existente")
        conn = get_conn()
        c = dict_cursor(conn)
        c.execute("SELECT legajo, apellido, nombre FROM colaboradores ORDER BY apellido")
        lista = c.fetchall()
        conn.close()

        opciones = ["(Nuevo)"] + [f"{r['legajo']} — {r['apellido']} {r['nombre']}" for r in lista]
        sel = st.selectbox("Seleccionar para editar (o dejar 'Nuevo')", opciones)

        datos = {}
        if sel != "(Nuevo)":
            leg = sel.split(" — ")[0]
            conn = get_conn(); c = dict_cursor(conn)
            c.execute("SELECT * FROM colaboradores WHERE legajo=%s", (leg,))
            row = c.fetchone()
            conn.close()
            if row: datos = dict(row)

        col1, col2 = st.columns(2)
        with col1:
            legajo   = st.text_input("Legajo *",    value=datos.get("legajo",""))
            apellido = st.text_input("Apellido *",  value=datos.get("apellido",""))
            nombre   = st.text_input("Nombre *",    value=datos.get("nombre",""))
            sector   = st.selectbox("Sector *", SECTORES,
                                    index=SECTORES.index(datos["sector"]) if datos.get("sector") in SECTORES else 0)
            tipo_col = st.selectbox("Tipo *", TIPOS,
                                    index=TIPOS.index(datos["tipo"]) if datos.get("tipo") in TIPOS else 0,
                                    format_func=lambda x: TIPO_LABEL.get(x,x))
        with col2:
            entrada  = st.text_input("Entrada (HH:MM)",  value=datos.get("entrada","09:00"))
            salida   = st.text_input("Salida (HH:MM)",   value=datos.get("salida","18:00"))
            ent_sab  = st.text_input("Entrada sábado",   value=datos.get("entrada_sab","09:00"))
            sal_sab  = st.text_input("Salida sábado",    value=datos.get("salida_sab","13:00"))
            alm_min  = st.number_input("Almuerzo (min)", value=int(datos.get("almuerzo_min",60)), min_value=0)
            brk_min  = st.number_input("Break (min)",    value=int(datos.get("break_min",0)),    min_value=0)
            rotativo = st.checkbox("Turno rotativo",     value=bool(datos.get("rotativo",0)))

        obs = st.text_area("Observaciones", value=datos.get("observaciones","") or "")

        if st.button("💾 Guardar", type="primary"):
            if not legajo or not apellido or not nombre:
                st.error("Legajo, Apellido y Nombre son obligatorios.")
            else:
                conn = get_conn(); c = dict_cursor(conn)
                if sel == "(Nuevo)":
                    c.execute("""INSERT INTO colaboradores
                        (legajo,apellido,nombre,sector,tipo,entrada,salida,entrada_sab,salida_sab,
                         almuerzo_min,break_min,rotativo,observaciones)
                        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                        ON CONFLICT (legajo) DO UPDATE SET
                        apellido=EXCLUDED.apellido, nombre=EXCLUDED.nombre""",
                        (legajo,apellido,nombre,sector,tipo_col,entrada,salida,
                         ent_sab,sal_sab,alm_min,brk_min,int(rotativo),obs))
                    log_auditoria(u["username"],"CREAR_COLABORADOR","colaboradores",None,f"Legajo {legajo}")
                else:
                    c.execute("""UPDATE colaboradores SET apellido=%s,nombre=%s,sector=%s,tipo=%s,
                        entrada=%s,salida=%s,entrada_sab=%s,salida_sab=%s,
                        almuerzo_min=%s,break_min=%s,rotativo=%s,observaciones=%s
                        WHERE legajo=%s""",
                        (apellido,nombre,sector,tipo_col,entrada,salida,
                         ent_sab,sal_sab,alm_min,brk_min,int(rotativo),obs,legajo))
                    log_auditoria(u["username"],"EDITAR_COLABORADOR","colaboradores",None,f"Legajo {legajo}")
                conn.commit(); conn.close()
                st.success("✅ Guardado correctamente")
                st.rerun()
