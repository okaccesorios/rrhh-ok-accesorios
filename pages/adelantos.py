...
def show():
    ...
    with tab0:
        ...
        st.markdown("""<div class="alert-info">
          ...
          <strong>Avisos de Ausencias</strong> → Novedades pendientes
        </div>""", unsafe_allow_html=True)
        archivo = st.file_uploader("Subí el Excel de Solicitudes", type=["xlsx","xls"], label_visibility="collapsed")
        if archivo:
    ...
    with tab1:
        ...
        conn = get_conn()
        c = dict_cursor(conn)
        sql = """SELECT a.*, c.apellido||' '||c.nombre as nombre
        ...
        if rows:
            ...
            for r in rows:
                ...
                with st.expander(f"{icon} **{r['nombre']}** — {TIPOS_LABEL.get(r['tipo'],r['tipo'])} | {monto_txt}"):
                    st.write(f"**Período:** {r['periodo']}  |  **Por:** {r['creado_por'] or '-'}")
    ...
    with tab2:
        ...
        if st.button("💾 Guardar", type="primary"):
            ...
            c.execute("INSERT INTO adelantos (legajo,periodo,tipo,monto,descripcion,creado_por) VALUES (%s,%s,%s,%s,%s,%s)",
