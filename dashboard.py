...
def show():
    ...
    sin_marcar_hoy = 0
    ausentes_hoy_list = []
    if hoy.weekday() < 6 and hoy.strftime("%Y-%m-%d") not in feriados_set:
    ...
    metricas = [
        ...
        (str(sin_marcar_hoy),                "Sin marcar hoy",                           "red" if sin_marcar_hoy else "green"),
    ...
    # ── SIN MARCAR HOY ────────────────────────────────────────
    with col_izq:
        st.markdown(f"#### ❌ Sin marcar hoy — {hoy.strftime('%d/%m/%Y')}")
        if hoy.weekday() == 6:
            st.info("Hoy es domingo.")
        elif hoy.strftime("%Y-%m-%d") in feriados_set:
            desc_feriado = next((r["descripcion"] for r in feriados_mes
                                  if r["fecha"] == hoy.strftime("%Y-%m-%d")), "Feriado")
            ...
            """, (hoy.strftime("%Y-%m-%d"), hoy.strftime("%Y-%m-%d"))).fetchall():
